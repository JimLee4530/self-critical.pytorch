...
class AdaAttCore(nn.Module):
    ...
    def forward(self, xt, att_feats, p_att_feats, avg_att_feats, state):
        h_out, p_out, state = self.lstm(xt, avg_att_feats, state)
        atten_out = self.attention(h_out, p_out, att_feats, p_att_feats)
        return atten_out, state


class AdaAtt(CaptionModel):
    def __init__(self, opt):
        super(AdaAtt, self).__init__()
        self.core = AdaAttCore(opt, opt.use_maxout)
        ...
        self.embed = nn.Sequential(nn.Embedding(self.vocab_size + 1, self.input_encoding_size),
                                nn.ReLU(),
                                nn.Dropout(self.drop_prob_lm))
        self.avg_embed = nn.Sequential(nn.Linear(self.att_feat_size, self.rnn_size),
                                    nn.ReLU(),
                                    nn.Dropout(self.drop_prob_lm))
    ...
    def forward(self, fc_feats, att_feats, num_bbox, seq):
        ...
        avg_att_feats = self.avg_embed(avg_att_feats)
        ...
        for i in range(seq.size(1) - 1):
            if self.training and i >= 1 and self.ss_prob > 0.0: # otherwiste no need to sample
                sample_prob = avg_att_feats.data.new(batch_size).uniform_(0, 1)
    ...
    def sample_beam(self, fc_feats, att_feats, num_bbox, opt={}):
        ...
        for k in range(batch_size):
            state = self.init_hidden(beam_size)
            # tmp_fc_feats = fc_feats[k:k+1].expand(beam_size, fc_feats.size(1))
            tmp_avg_att_feats = avg_att_feats[k:k + 1].expand(beam_size, avg_att_feats.size(1))
    ...
    def sample(self, fc_feats, att_feats, num_bbox, opt={}):
        ...
        for t in range(self.seq_length + 1):
            if t == 0: # input <bos>
                it = avg_att_feats.data.new(batch_size).long().zero_()
            elif sample_max:
