...
class TopDownCore(nn.Module):
    def __init__(self, opt):
        ...
        #build s
        self.i2gt = nn.Linear(self.input_encoding_size, self.rnn_size1)
        self.h12gt = nn.Linear(self.rnn_size1, self.rnn_size1)
        self.s2att = nn.Linear(self.rnn_size1, 512)
        self.h12att = nn.Linear(self.rnn_size1, 512)
        self.satt = nn.Linear(512, 1)
        self.imgatt_emb = nn.Linear(self.att_feat_size, 512)

        #build attention
        self.h2emb = nn.Linear(self.rnn_size1, self.att_hid_size)
        self.attfc1 = nn.Linear(self.att_hid_size, 512)
        self.attfc2 = nn.Linear(512, 1)

        #build language lstm
        # self.att2h2 = nn.Linear(self.att_feat_size, 5 * self.rnn_size2)
        self.att2h2 = nn.Linear(512, 5 * self.rnn_size2)
    ...
    def forward(self, xt, att_feats, p_att_feats, avg_att_feats, state):
        '''
        ...
        # build s
        gate_s = F.sigmoid(self.i2gt(xt) + self.h12gt(state[0][-1]))
        s = gate_s * F.tanh(next_c1)
        ...
        s_att = self.satt(F.tanh(self.s2att(s) + self.h12att(next_h1)))
        dot = torch.cat([dot, s_att], 1)

        weight = F.softmax(dot)
        beta = weight.narrow(1, att_size, 1)
        weight = weight.narrow(1, 0, att_size)
        ...
        att_res = self.imgatt_emb(att_res)
        att_res = att_res * (1 - beta) + beta * s
...
class TopDownAdaattModel(CaptionModel):
    def __init__(self, opt):
        super(TopDownAdaattModel, self).__init__()
    ...
    def forward(self, fc_feats, att_feats, num_bbox, seq):
        ...
        for i in range(seq.size(1) - 1):
            ...
            output, state = self.core(xt, att_feats, p_att_feats, avg_att_feats, state)
            output = F.log_softmax(self.logit(output))
            outputs.append(output)

        return torch.cat([_.unsqueeze(1) for _ in outputs], 1)

    def single_sample(self, fc_feats, att_feats, num_bbox, opt={}):
        batch_size = att_feats.size(0)
        ...
        self.tmp_avg_att_feats = avg_att_feats
        self.tmp_att_feats = att_feats
        self.tmp_p_att_feats = p_att_feats
    ...
    def get_logprobs_state(self, it, tmp_att_feats, tmp_p_att_feats, tmp_avg_att_feats, state):
        # 'it' is Variable contraining a word index
        xt = self.embed(it)

        output, state = self.core(xt, tmp_att_feats, tmp_p_att_feats, tmp_avg_att_feats, state)
        logprobs = F.log_softmax(self.logit(output))

        return logprobs, state

    def single_sample_beam(self, batch_ind, fc_feats, att_feats, num_bbox, opt={}):
        beam_size = opt.get('beam_size', 1)
        sample_max = opt.get('sample_max', 1)
        temperature = opt.get('temperature', 1.0)
        ...
        tmp_avg_att_feats = avg_att_feats[batch_ind:batch_ind + 1].expand(beam_size, avg_att_feats.size(1))
        tmp_att_feats = att_feats[batch_ind:batch_ind + 1].expand(*((beam_size,) + att_feats.size()[1:])).contiguous()
        tmp_p_att_feats = p_att_feats[batch_ind:batch_ind + 1].expand(*((beam_size,) + p_att_feats.size()[1:])).contiguous()
        self.tmp_avg_att_feats = tmp_avg_att_feats
        self.tmp_att_feats = tmp_att_feats
        self.tmp_p_att_feats = tmp_p_att_feats
    ...
    def sample_beam(self, fc_feats, att_feats, num_bbox, opt={}):
        ...
        for k in range(batch_size):
            ...
            self.done_beams[k] = self.beam_search(state, logprobs, tmp_att_feats, tmp_p_att_feats, tmp_avg_att_feats, opt=opt)
            seq[:, k] = self.done_beams[k][0]['seq'] # the first beam has highest cumulative score
            seqLogprobs[:, k] = self.done_beams[k][0]['logps']
        # return the samples and their log likelihoods
        return seq.transpose(0, 1), seqLogprobs.transpose(0, 1)
